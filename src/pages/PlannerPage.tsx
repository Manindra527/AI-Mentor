...
const shiftPlanForwardForHoliday = (
  ...
  return Object.entries(currentPlan).reduce<Record<string, TimeBlock[]>>((nextPlan, [date, blocks]) => {
    const targetDate = date >= holidayDate ? addDays(date, 1) : date;
      ...
      <div className="flex-1 min-h-0 overflow-y-auto hide-scrollbar space-y-3 pb-28">
        ...
        {blocks.length > 0 ? (
          blocks.map((block) => {
            ...
            return (
              <div key={block.id} className="bg-card rounded-2xl shadow-card p-4 flex items-start gap-3">
                ...
                <div className="flex flex-col gap-2">
                  {!locked ? (
                    ...
                    <button
                      ...
                      className="rounded-xl px-3 py-2 bg-accent text-xs font-semibold text-foreground hover:bg-primary/10 transition-colors"
                    >
                      Postpone
                    </button>
                  )}
        ...
        <DialogContent className="max-w-sm rounded-2xl">
          ...
          <div className="space-y-3 pt-2">
            {["Festival", "Travel", "Health Break"].map((reason) => (
              <button
                key={reason}
                onClick={() => handleMarkHoliday(reason)}
