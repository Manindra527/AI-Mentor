...
const monthlyTrend = [
  { week: "W1", hours: 22, completion: 85 },
  { week: "W2", hours: 28, completion: 92 },
  { week: "W3", hours: 19, completion: 72 },
  { week: "W4", hours: 24, completion: 82 },
];

const mockScores = [
  { test: "Mock 1", score: 62 },
  { test: "Mock 2", score: 58 },
  { test: "Mock 3", score: 71 },
  { test: "Mock 4", score: 75 },
  { test: "Mock 5", score: 80 },
];

const moodData = { Focused: 12, Stressed: 4, Tired: 3, Motivated: 6, Distracted: 2, Frustrated: 1 };
    ...
    <div className="space-y-5 pb-4">
      ...
      <div className="grid grid-cols-2 gap-3">
        <div className="bg-card rounded-2xl shadow-card p-4">
          ...
          <p className="text-2xl font-bold text-foreground">{tab === "daily" ? "4.2h" : tab === "weekly" ? "24.5h" : "92h"}</p>
          <p className="text-xs text-muted-foreground font-medium">Study Hours</p>
        </div>
        <div className="bg-card rounded-2xl shadow-card p-4">
          <Target size={18} className="text-primary mb-1" />
          <p className="text-2xl font-bold text-foreground">{tab === "daily" ? "100%" : tab === "weekly" ? "82%" : "84%"}</p>
        ...
        <div className="bg-card rounded-2xl shadow-card p-4">
          ...
          <p className="text-xs text-muted-foreground font-medium">Weekly Consistency</p>
        </div>
      </div>
      ...
      <div className="bg-card rounded-2xl shadow-card p-5">
        ...
        <div className="h-40">
          <ResponsiveContainer width="100%" height="100%">
            <BarChart data={tab === "monthly" ? monthlyTrend : weeklyData}>
              <XAxis dataKey={tab === "monthly" ? "week" : "day"} axisLine={false} tickLine={false} tick={{ fontSize: 11, fill: "hsl(20 8% 50%)" }} />
              <YAxis hide />
              <Tooltip
                contentStyle={{ borderRadius: "12px", border: "none", boxShadow: "var(--shadow-md)", fontSize: "12px" }}
              />
      ...
      <div className="grid grid-cols-2 gap-3">
        <div className="bg-card rounded-2xl shadow-card p-4">
          ...
          <p className="font-bold text-foreground">Reasoning</p>
          <p className="text-xs text-muted-foreground mt-1">Accuracy: 62%</p>
        </div>
        <div className="bg-card rounded-2xl shadow-card p-4">
          <p className="text-xs font-semibold text-success mb-2">Strong Subject</p>
          <p className="font-bold text-foreground">English</p>
          <p className="text-xs text-muted-foreground mt-1">Accuracy: 89%</p>
        </div>
      </div>

      {/* Mood Trend */}
      <div className="bg-card rounded-2xl shadow-card p-5">
        <h3 className="font-semibold text-foreground mb-3">Weekly Mood Analysis</h3>
